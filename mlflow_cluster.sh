...
u="jklassen"
c=5000  # MLflow default port
l=5000  # Local port for MLflow
s="/home/jklassen/forensics_TOFscreenings/01_aging_study/modeling/ml_project/mlflow_job.sh"
w=10
k="empty"
error="empty"
out="empty"
...
jobname=`ssh ${u}@login.genome.au.dk 'grep "^#SBATCH -J\|^#SBATCH --job-name" '${s}' | awk -F= '\''{print $2}'\'' | awk '\''{print $1}'\'''`
printf "   Job name: ${jobname}\n"
p=`ssh ${u}@login.genome.au.dk 'squeue -n '${jobname}' -u '${u}' -h | wc -l'`

if [ ${p} -gt 0 ];
then
	printf "   Found ${p} jobs with the same name ${jobname}...\n"
	ssh ${u}@login.genome.au.dk 'for job in `squeue -n '${jobname}' -u '${u}' -h -o "%i"`; do echo "      - Canceling job ${job}"; scancel ${job}; done';
else
...
#If the job is running...
    ...
    # Open new firefox window to the MLflow UI
    
    firefox --new-window "https://localhost:${l}/" 
